...
import { Booking, NotificationDetails } from '../types';

interface InfoSectionsProps {
  onTrack: (code: string) => Booking | undefined;
  onCancelRequest: (booking: Booking) => void;
  showNotification: (details: NotificationDetails) => void;
}

const InfoSections: React.FC<InfoSectionsProps> = ({ onTrack, onCancelRequest, showNotification }) => {
  const [openSection, setOpenSection] = useState<string | null>('address'); // Default open
  ...
  const handleTrack = () => {
    ...
    const booking = onTrack(trackingCode);
    if (booking) {
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="input-wrapper flex-grow min-w-0">
                <input
                  ...
                  className="w-full p-3 bg-black/20 text-white border-2 border-transparent focus:border-cyan-400 rounded-lg focus:outline-none transition"
                />
              </div>
              <button
                type="button"
                onClick={handleTrack}
                className="bg-gradient-to-br from-cyan-500 to-teal-500 text-white font-bold py-3 px-6 rounded-xl shadow-lg shadow-cyan-500/30 transform hover:-translate-y-1 transition-all duration-300"
              >
          ...
          {foundBooking && (
            <div className="p-5 bg-black/20 border border-white/10 rounded-xl shadow-lg animate-fade-in space-y-4">
              ...
              {!isConfirmingCancel ? (
                <button
                  ...
                  className="w-full mt-4 bg-gradient-to-br from-red-500 to-orange-500 text-white font-bold py-3 px-4 rounded-lg shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all"
                >
                  <i className="fas fa-trash-alt mr-2"></i> درخواست لغو نوبت
                </button>
              ) : (
                <div className="mt-4 p-4 bg-red-500/10 border-2 border-dashed border-red-500/30 rounded-lg text-center space-y-3">
                  ...
                  <div className="flex justify-center gap-4">
                    ...
                    <button onClick={() => setIsConfirmingCancel(false)} className="bg-slate-600 text-slate-100 font-bold py-2 px-6 rounded-lg transition hover:bg-slate-500">
                      انصراف
                    </button>
                  </div>
                </div>
  ...
  return (
    <section className="mt-12 pt-8 border-t border-fuchsia-500/20 space-y-3 fade-in-up" style={{ animationDelay: '1200ms' }}>
      {sections.map(({ id, title, icon, content }) => (
        <div key={id} className="bg-black/20 rounded-xl overflow-hidden transition-all duration-500 ease-in-out shadow-md backdrop-blur-sm border border-white/5">
          <button
            onClick={() => toggleSection(id)}
            className="w-full flex justify-between items-center p-4 text-right font-bold text-slate-200 hover:bg-white/5 transition-colors"
          >
            <span className="text-lg"><i className={`fas ${icon} ml-3 text-fuchsia-400`}></i>{title}</span>
            <i className={`fas fa-chevron-down transition-transform duration-300 text-slate-400 ${openSection === id ? 'rotate-180' : ''}`}></i>
          </button>
          <div
            className={`transition-all duration-500 ease-in-out grid ${openSection === id ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}
          >
